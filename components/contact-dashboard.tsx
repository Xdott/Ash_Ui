...
export default function ContactDashboard() {
  ...
  const filteredContacts = useMemo(() => {
    ...
    if (searchTerm.trim() || filters.search.trim()) {
      const searchLower = (searchTerm || filters.search).toLowerCase().trim()
    ...
    if (filters.status.length > 0) {
      filtered = filtered.filter((contact) => filters.status.includes(contact.lead_status || "new"))
    }

    // Apply source filters
    if (filters.source.length > 0) {
      filtered = filtered.filter((contact) => filters.source.includes(contact.lead_source || "import"))
    }

    // Apply job title filters
    if (filters.jobTitle.length > 0) {
      filtered = filtered.filter((contact) =>
        filters.jobTitle.some((title) => contact.job_title?.toLowerCase().includes(title.toLowerCase())),
      )
    }
  ...
  const fetchAllContacts = async (forceRefresh = false) => {
    if (!user?.email) return

    setInitialLoading(true)

    try {
      console.log(`🔄 Fetching all contacts ${forceRefresh ? "with force refresh" : "normally"}...`)
      ...
      // Use ONLY the contact-enrichment endpoint
      ...
      if (result.contacts && Array.isArray(result.contacts)) {
        ...
        console.log(`✅ Loaded ${transformedContacts.length} contacts ${forceRefresh ? "(cache cleared)" : ""}`)

        if (forceRefresh) {
          toast({
            title: "Data Refreshed",
            description: `Refreshed ${transformedContacts.length} contacts with latest data`,
          })
        }
      } else {
        console.warn("No contacts array found in response")
        setAllContacts([])
      }
    } catch (error) {
  ...
  // Add this function after the fetchAllContacts function
  const handleRefreshClick = () => {
    fetchAllContacts(true) // Pass true to force refresh and clear cache
  }
  ...
  const getActiveFilterCount = () => {
    let count = 0
    if (searchTerm || filters.search) count++
  ...
  const validateEmails = async () => {
    ...
    try {
      ...
      setSelectedContacts(new Set())
      await fetchAllContacts(false) // Refresh data
    } catch (error) {
  ...
  return (
    <div className="h-screen flex bg-gray-50">
      ...
      <div className={`flex-1 flex flex-col transition-all duration-300 ${isFilterSidebarOpen ? "mr-80" : "mr-0"}`}>
        {/* Header */}
        <header className="bg-white border-b border-gray-200 px-4 py-3">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-xl font-semibold text-gray-900">Contact Dashboard</h1>
            ...
            <div className="flex items-center gap-2 text-sm text-gray-500">
              ...
              <Button
                variant="outline"
                onClick={handleRefreshClick} // Use the force refresh function
        ...
        <div className="bg-white border-b border-gray-200 px-4 py-2">
          <div className="flex items-center gap-2">
            ...
            <Button variant="outline" size="sm" onClick={() => setIsFilterSidebarOpen(true)}>
                ...
                {paginatedContacts.map((contact, index) => (
                  ...
                  >
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-1 text-xs text-blue-600">
                            {inlineEditingField?.contactId === contact.id && inlineEditingField.field === "email" ? (
                              <Input
                                value={inlineEditValue}
                                onChange={(e) => setInlineEditValue(e.target.value)}
                                className="h-6 py-0 px-1 text-xs"
                    ...
                    <TableCell>
                      <div>
                        <div className="font-medium text-sm">
                          {inlineEditingField?.contactId === contact.id && inlineEditingField.field === "company" ? (
                            <Input
                              value={inlineEditValue}
                              onChange={(e) => setInlineEditValue(e.target.value)}
                              className="h-6 py-0 px-1 text-xs"
                        ...
                        <div className="text-xs text-gray-500">
                          {inlineEditingField?.contactId === contact.id && inlineEditingField.field === "job_title" ? (
                            <Input
                              value={inlineEditValue}
                              onChange={(e) => setInlineEditValue(e.target.value)}
                              className="h-6 py-0 px-1 text-xs"
                    ...
                    <TableCell>
                      {inlineEditingField?.contactId === contact.id && inlineEditingField.field === "phone" ? (
                        ...
                        <div
                          ...
                          onClick={() => startInlineEdit(contact.id, "phone", contact.phone || contact.mobile || "")}
                        >
        ...
        {!initialLoading && paginatedContacts.length > 0 && (
          <div className="bg-white border-t border-gray-200 px-4 py-2">{renderPagination()}</div>
        )}
      </div>
      ...
      <SaveSearchDialog
        ...
        onSave={(name, description) => {
          ...
          existingSearches.push(savedSearch)
          localStorage.setItem("savedSearches", JSON.stringify(existingSearches))

          toast({
            title: "Search Saved",
            description: `"${name}" has been saved as a report. View it in the Reports tab.`,
          })
